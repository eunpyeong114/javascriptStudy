import axios from "axios";
import cheerio from "cheerio";

const createMessage = async (mymessage) => {
  // 1. 입력된 메세지에서 http로 시작하는 문장이 있는지 먼저 찾기! (.find() 등의 알고리즘 사용하기)
  const url = "https://www.naver.com/";

  // 2. axios.get으로 요청해서 html코드 받아오기 => 스크래핑
  const result = await axios.get(url);
  console.log(result.data);

  // 3. 스크래핑 결과에서 OG(오픈그래프) 코드를 골라내서 변수에 담기 => cheerio 도움 받기
  const $ = cheerio.load(result.data);
  //cheerio의 반복문 구문 .each
  // index 번호와 해당 번호의 메타 태그가 반복되어 들어온다
  // 그럼 el (=메타 태그) 안에 OG가 들어가 있는가 ?
  $("meta").each((index, el) => {
    if ($(el).attr("property") && $(el).attr("property").includes("og:")) {
      const key = $(el).attr("property"); // og:title, og:description ...
      const value = $(el).attr("content"); //    네이버 ,  네이버 메인에서 다양한 정보와...
      console.log(key, value);
    } else {
      // og: 없는 애들 무시
    }
  });
};

createMessage("안녕하세요~ https://naver.com에 방문해 주세요!");
