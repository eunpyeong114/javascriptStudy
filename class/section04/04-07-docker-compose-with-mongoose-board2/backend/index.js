...
mongoose.set("debug", true); //몽구스 코드(Board.find()) -> 몽고 DB코드(db.board.find()) log로 변경 결과를 볼 수 있게 해줌

mongoose
  .connect("mongodb://my-database:27017/mydocker")
  .then(() => {
    console.log("db 접속에 성공하였습니다222.");
  })
  .catch(() => {
...
app.get("/boards", async function (req, res) {
  ...
  //   },
  // ];
  /// Board 통에서 조회해서 가져온 결과값 result에 할당
...
app.post("/boards", async function (req, res) {
  ...
  /// 2. DB에 접속 후, 데이터를 저장 => 데이터를 '저장'
  /// 새로운 통을 만들기
  /// board.model.js에 있는 Board의 틀대로만 넣을 수 있다. 바꾸려면 Board의 틀 자체를 바꿔야함
  const board = new Board({
    writer: req.body.writer,
    title: req.body.title,
    contents: req.body.contents,
  });
  /// ODM 몽구스를 통해 긴 명령어를 간단하게 바꿔서 데이터베이스에 등록을 해줌. 저장해주는 거 기다려줘야됨
  await board.save();
